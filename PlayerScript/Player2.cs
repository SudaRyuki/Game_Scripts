...
public class Player2 : MonoBehaviourPunCallbacks, IPunObservable
{
    #region コンポーネント参照
    ...
    [Header("Movement")]
    [SerializeField] private float speed = 1.0f; [SerializeField] private float jumpForce = 5.0f;
    ...
    #region Attack Settings[Header("Attack")]
    ...
    #region Layer & UI & Effects Settings[Header("レイヤー設定")]
    public LayerMask stageLayer;
    public LayerMask opponentLayer; [Header("UI / Indicators")]
    public GameObject youIndicator;
    public SpriteRenderer teamIndicatorSprite; [Header("Audio")]
    ...
    private IEnumerator DelayedAttackCoroutine()
    {
        ...
        if (!PhotonNetwork.InRoom || PhotonNetwork.IsMasterClient || photonView.IsMine)
        {
            ...
            {
                Collider2D[] hitObjects = Physics2D.OverlapBoxAll(attackPoint.position, new Vector2(attackRange * 2, attackRange), 0, opponentLayer);
                foreach (Collider2D hit in hitObjects)
                {
                    Player opponent = hit.GetComponent<Player>(); // Player2はPlayer1を攻撃する
                    if (opponent != null && opponent.teamID != this.teamID && !hitOpponents.Contains(opponent.gameObject))
                    {
                        if (opponent.IsVulnerable())
                        {
                            ...
                            {
                                opponent.GetComponent<PhotonView>().RPC(nameof(Player.TakeDamage), RpcTarget.All);
    ...
    private IEnumerator FireSpecialLaserCoroutine()
    {
        ...
        if (specialBeamPrefab != null && alienBeamSpawnPoint != null)
        {
            ...
            if (laser != null)
            {
                ...
                if (laserScript != null) laserScript.shooterTeamID = this.teamID;
            }
        }
    ...
    public void AddSpecialPoint(int amount)
    {
        if (!PhotonNetwork.InRoom || photonView.IsMine)
        {
            ...
            UpdateSpecialGaugeUI();
            Debug.Log($"必殺技ポイント: {currentSpecialPoints} / {maxSpecialPoints}");
        }
    }
    ...
    public void TakeDamage()
    {
        ...
        {
            if (ScoreManager.instance.GetScore("Player2") > 0)
            {
    ...
    private void ResetStatus()
    {
        ...
        animator.SetBool("Jump", false); // "IsJumping"から"Jump"に統一しました
        animator.SetBool("IsShielding", false);
        animator.SetBool("IsStunned", false);
        animator.SetFloat("Speed", 0);
    }
    ...
    private void RpcActivateAlienPower()
    {
        ...
        hasAlienPower = true;
        Debug.Log(gameObject.name + "がエイリアンパワーを獲得！");
    ...
    [PunRPC]
    private void RpcNotifyItemPickup(int item_ViewID, int player_ViewID)
    {
        GameObject itemObject = PhotonView.Find(item_ViewID)?.gameObject;
        PhotonView playerPhotonView = PhotonView.Find(player_ViewID);
